  ...
  if (sorted.length === 0) {
    return (
      <div className="bg-white border border-[#dddddd] rounded-sm px-6 py-12 text-center">
        <p className="text-[#6b7280]">No exceptions in this period — clean close.</p>
      </div>
    );
  }
      ...
      {sorted.map((ex) => (
        ...
        >
          ...
          <div className="shrink-0 flex gap-2">
            <button className="text-xs px-3 py-1.5 border border-[#dddddd] rounded-sm text-[#4B4F58] hover:border-[#0170B9] hover:text-[#0170B9] transition-colors">
              Resolve
            </button>
            <button className="text-xs px-3 py-1.5 border border-[#dddddd] rounded-sm text-[#4B4F58] hover:border-[#6b7280] transition-colors">
              Won&apos;t fix
            </button>
          </div>
        </div>
