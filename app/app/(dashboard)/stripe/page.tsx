...
import type { StripeCharge } from "@/lib/types";
import { april2026PaidCharges } from "@/lib/mock/stripe-transactions";
import { april2026FailedCharges } from "@/lib/mock";
import { clientDatabase } from "@/lib/mock";
import { formatMoney, formatDate } from "@/lib/format";
import { cn } from "@/lib/utils";
import { Search } from "lucide-react";

// ── Enrich charges with client info ─────────────────────────

type EnrichedCharge = StripeCharge & { display_name: string; batch: string };

const ALL_CHARGES: EnrichedCharge[] = [
  ...april2026PaidCharges,
  ...april2026FailedCharges,
]
  .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
  .map((c) => {
    const client = clientDatabase.find(
      (cl) => cl.stripe_id && cl.stripe_id === c.stripe_id
    );
    return {
      ...c,
      display_name: client?.display_name ?? c.customer_email,
      batch: client?.batch ?? "—",
    };
  });
...
function StatusBadge({ status }: { status: StripeCharge["status"] }) {
  const styles: Record<StripeCharge["status"], string> = {
...
export default function StripeTransactionsPage() {
  ...
  const batches = [...new Set(ALL_CHARGES.map((c) => c.batch).filter((b) => b !== "—"))].sort();

  const filtered = ALL_CHARGES.filter((c) => {
      ...
      <div className="flex items-baseline justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-[#6b7280] mt-0.5">
            April 2026 · {filtered.length} of {ALL_CHARGES.length} charges
          </p>
        </div>
        {/* KPI chips */}
        ...
        <table className="w-max min-w-full text-xs border-collapse">
          <thead>
            <tr className="bg-[#F5F5F5] border-b border-[#dddddd]">
              ...
              <th className="px-3 py-2 text-left font-medium text-[#6b7280]">Decline Reason</th>
              <th className="px-3 py-2 text-left font-medium text-[#6b7280] whitespace-nowrap">Invoice ID</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((c) => (
              ...
              >
                ...
                <td className="px-3 py-2 text-[#6b7280] max-w-[160px]">
                  <span className="truncate block" title={c.decline_reason ?? ""}>
                    {c.decline_reason ?? "—"}
                  </span>
                </td>
                <td className="px-3 py-2 font-mono text-[10px] text-[#6b7280] whitespace-nowrap">
                  {c.invoice_id ? c.invoice_id.slice(0, 22) + "…" : "—"}
                </td>
              </tr>
            ))}
            {filtered.length === 0 && (
