...
function DirectoryTab() {
  // Editable in-memory copy of the client list
  const [clients, setClients] = useState<ClientRecord[]>([...clientDatabase]);
...
function HistoryTab() {
  const periodRows = PERIODS.map((p) => {
    ...
    const won     = clientDatabase.filter((c) => c.start_date && c.start_date >= p.start_date && c.start_date <= p.end_date);
    ...
    <div className="px-6 py-6 space-y-5 max-w-[1600px]">
      <div>
        ...
        <p className="text-sm text-[#6b7280] mt-0.5">
          {clientDatabase.length} clients · Stripe IDs, billing plans, and lifecycle history
        </p>
      </div>
