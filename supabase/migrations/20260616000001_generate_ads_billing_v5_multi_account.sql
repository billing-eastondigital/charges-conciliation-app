-- Migration 20260616000001 — generate_ads_billing v5
-- Fix: sum spend across ALL Google Ads accounts for a client (primary + additional IDs
-- stored in client_platform_ids.other_ids->>'google_ads_additional_customer_ids').
-- Fixes cus_MAQFq6FlG4sGc3 (Real Estate Posts + Hall Signs) being OVERPAID because
-- only the primary account (4631988316) was billed, not the additional one (8378921672).
    ...
    BEGIN
      ...
      WITH spend AS (
        ...
        WHERE period_label = p_period_label
          AND google_ads_customer_id = ANY(v_all_ids)
      ),
      calc AS (
        SELECT s.*,
...
-- DOWN: restore v4 (single-account, no v_all_ids)
