-- Migration 20260617000004 — generate_ads_billing v9
-- Fix line item date range: show the actual campaign data window (previous month)
-- instead of the billing period dates. Mirrors ingest-google-ads date logic:
--   billing_day_one = 1  → full previous calendar month
--   billing_day_one = N  → day N of prev month to day N-1 of current month
-- v_date_range is now computed per-client inside the loop.
...
BEGIN
  ...
  -- First and last day of the calendar month preceding the period
  ...
  LOOP
    BEGIN
      ...
      -- Compute actual data window (mirrors ingest-google-ads logic)
...
-- DOWN: restore v8 (see 20260617000003)
