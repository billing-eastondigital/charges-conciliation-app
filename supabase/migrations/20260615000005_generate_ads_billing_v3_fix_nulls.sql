-- Migration 20260615000005 — generate_ads_billing v3 final
-- Fix: COALESCE on all SUM aggregates so clients with no spend data return 0 (not NULL).
-- Clients with campaign_count = 0 are skipped — they reconcile as MISSING_PAYMENT.
-- This is the production version of v3 (replaces 20260615000004).
...
BEGIN
  ...
  FOR rec IN
    SELECT cap.stripe_id, cap.display_name, cap.billing_method, cap.base_fee, cap.billing_percentage,
  ...
  LOOP
    BEGIN
      ...
      WITH spend AS (
        ...
        WHERE period_label = p_period_label AND google_ads_customer_id = rec.google_ads_customer_id
      ),
      calc AS (
        SELECT s.*,
      ...
      INSERT INTO expected_charges (period_label, stripe_id, account_name, expected_amount, source, billing_detail)
      SELECT
        p_period_label, rec.stripe_id, rec.display_name,
...
-- DOWN: revert to v2 (migration 20260615000002)
