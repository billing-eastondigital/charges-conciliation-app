-- Migration 20260617000001 — google_ads_campaign_overrides
--
-- Stores manual campaign-level billing exclusions.
-- Separate from google_ads_spend so overrides survive the daily delete+reinsert cycle.
-- Keyed by (period_label, google_ads_customer_id, campaign_id).
--
-- generate_ads_billing v6 reads this table to exclude campaigns from billing calcs.

CREATE TABLE google_ads_campaign_overrides (
  ...
  UNIQUE (period_label, google_ads_customer_id, campaign_id)
);

CREATE INDEX google_ads_campaign_overrides_period_idx
  ON google_ads_campaign_overrides (period_label);

ALTER TABLE google_ads_campaign_overrides ENABLE ROW LEVEL SECURITY;

CREATE POLICY "anon_read_campaign_overrides"
  ON google_ads_campaign_overrides FOR SELECT TO anon USING (true);

CREATE POLICY "anon_write_campaign_overrides"
  ON google_ads_campaign_overrides FOR ALL TO anon USING (true) WITH CHECK (true);

COMMENT ON TABLE google_ads_campaign_overrides IS
  'Manual billing exclusions per campaign per period. Survives re-ingest. Read by generate_ads_billing to cap billing for specific campaigns.';


-- ── generate_ads_billing v6 ───────────────────────────────────────────────
-- Adds NOT EXISTS filter against google_ads_campaign_overrides so manually
-- excluded campaigns are removed from the billing calculation.
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Build array of all customer IDs: primary + additional from other_ids
      ...
      WITH spend AS (
        SELECT
          -- Shopping/PMax/Display/Video channels (excluding brand + manual overrides)
          COALESCE(SUM(CASE
            ...
            THEN conversion_value ELSE 0 END), 0) AS shopping_revenue,
          COALESCE(SUM(CASE
            WHEN channel_type IN (3,4,6,10)
             AND NOT (campaign_name = ANY(shopping_exclusions))
             AND NOT EXISTS (
            ...
            THEN cost_usd ELSE 0 END), 0) AS shopping_cost,
          -- Search channel (excluding brand + manual overrides)
          COALESCE(SUM(CASE
            WHEN channel_type = 2
            ...
            THEN conversion_value ELSE 0 END), 0) AS search_revenue,
          COALESCE(SUM(CASE
            WHEN channel_type = 2
             AND NOT (campaign_name = ANY(search_exclusions))
             AND NOT EXISTS (
            ...
            THEN cost_usd ELSE 0 END), 0) AS search_cost,
...
-- DOWN:
-- DROP TABLE google_ads_campaign_overrides;
-- Restore v5 of generate_ads_billing (see 20260616000001).
